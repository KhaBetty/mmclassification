...
class MultiRandomErasing(object):
    ...
    def __init__(self,
        ...
        assert mode in ['const', 'rand']
        if isinstance(fill_color, Number):
            fill_color = [fill_color] * num_channels
        assert isinstance(fill_color, Sequence) and len(fill_color) == num_channels \
            and all(isinstance(x, Number) for x in fill_color), \
            'fill_color should be a float or Sequence with three int.'
        if fill_std is not None:
            if isinstance(fill_std, Number):
                fill_std = [fill_std] * num_channels
            assert isinstance(fill_std, Sequence) and len(fill_std) == num_channels \
                and all(isinstance(x, Number) for x in fill_std), \
                'fill_std should be a float or Sequence with three int.'
    ...
    def _fill_pixels(self, img, top, left, h, w):
        if self.mode == 'const':
            patch = np.empty((h, w, self.num_channels), dtype=np.uint8)
            patch[:, :] = np.array(self.fill_color, dtype=np.uint8)
        elif self.fill_std is None:
            # Uniform distribution
            patch = np.random.uniform(0, 256, (h, w, self.num_channels)).astype(np.uint8)
        else:
            # Normal distribution
            patch = np.random.normal(self.fill_color, self.fill_std, (h, w, self.num_channels))
            patch = np.clip(patch.astype(np.int32), 0, 255).astype(np.uint8)

        # TODO erase , case for specific error - for one channel
        if len(img.shape) == 2:
